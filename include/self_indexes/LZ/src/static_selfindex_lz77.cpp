...
static_selfindex_lz77* static_selfindex_lz77::build(char* filename, char* filenameidx, unsigned char binaryRev, unsigned char binarySst, unsigned char store_sst_ids){
    ...
    //0) Save the header
    unsigned char hdr = SELFINDEX_LZ77_HDR;
    ...
    unsigned int* ids = pat_tree->leafIds();//size = patricia->ids()
    //delete [] ids;
...
unsigned int static_selfindex_lz77::size(){
    return sizeof(static_selfindex_lz77) + 
    sigma_mapper->size() +
...
unsigned int static_selfindex_lz77::save(FILE *fp){
    return 1;
}
/** Loads the index from a file */
static_selfindex_lz77* static_selfindex_lz77::load(FILE * fp){
    unsigned char hdr;
    if(fread(&hdr,sizeof(unsigned char),1,fp)!=1 || hdr != SELFINDEX_LZ77_HDR){
        std::cerr<<"ERROR: load: wrong header"<<std::endl;
        return NULL;
    }
    static_selfindex_lz77* ret = new static_selfindex_lz77();
    /**************************************************************************/
    /* Load Options                                                           */
...
bool static_selfindex_lz77::_exist(unsigned char* pattern, unsigned int plen){
...
void static_selfindex_lz77::_primaryOcc(unsigned char* pattern, unsigned int plen, std::vector<unsigned int>* occ_pos, unsigned int max_occs){
    //special case
...
void static_selfindex_lz77::_computeSpecialOcc(unsigned char* pattern, unsigned char* rev_pattern,unsigned int plen, std::vector<unsigned int>* occ_pos){
    unsigned int Ll,Lr;
    if(binaryRev){
    ...
    if(Lr>=Ll){
        ...
        if(strcmp((char*)extract_str,(char*)pattern) != 0){//no match so we discard all range occs
            ...
            //std::cout<<"***Special Occ found"<<std::endl;
...
void static_selfindex_lz77::_computePrimaryOcc(unsigned char* pattern, unsigned char* rev_patternL, unsigned int plenL, unsigned char* patternR, unsigned int plenR, std::vector<unsigned int>* occ_pos){
    ...
    if(Lr>=Ll){
        ...
        if(Rr>=Rl){
            std::vector<unsigned int>* range_res = this->range->search(Rl,Rr,Ll,Lr);//los rangos son desde 1, es por eso que serachsst no le resta 1 al L y R
            if(range_res->size()>0){
                unsigned int phrase_len;
...
void static_selfindex_lz77::_searchTreeSst(unsigned char* pattern, unsigned int plen, unsigned int* L, unsigned int* R){
...
void static_selfindex_lz77::_searchTreeRev(unsigned char* pattern, unsigned int plen, unsigned int* L, unsigned int* R){
    unsigned int node = 1;//start from root
    unsigned int pos=0;
    unsigned int s_pos;
    while(pos<plen && !rev_trie->inspect(node)){
...
void static_selfindex_lz77::_secondaryOcc(unsigned int plen, std::vector<unsigned int>* occ_pos, unsigned int max_occs){
...
void static_selfindex_lz77::_computeSecondaryOcc(unsigned int pos, unsigned int plen, std::vector<unsigned int>* occ_pos, unsigned int max_occs){
    ...
    while(1){
        ...
        while(source_start+s_length-1 >= pos+plen){
            ...
            //ATTENTION: perm->rev_pi(source_id) should never be 0.
            //perm->rev_pi(x) = 0 <=> x = 0, thus length(x) = 1 so it is not covering anything
...
unsigned char* static_selfindex_lz77::_display(unsigned int start, unsigned int end){
...
void static_selfindex_lz77::_charextractLZ77(unsigned char* answer, unsigned start, unsigned end, unsigned int offset_start, unsigned int offset_end){
    while(1){//start<=end
...
int static_selfindex_lz77::_source(unsigned int i){
    unsigned int r = perm->pi(i)+1;
    int pos = this->sources->select(r);
    return pos-r;
}
unsigned int static_selfindex_lz77::_source_length(unsigned int i){
    //if(i==0)return 1;
    unsigned int length;
...
void static_selfindex_lz77::_binarySearchRev(unsigned char* pattern, unsigned int plen, unsigned int* left, unsigned int* right){
    int lower = 0;
    ...
    if(lower<=upper){//found
        ...
        *left=upper_a+1;//lower_a
        ...
        while(lower_a <= upper_a){
            ...
            unsigned int phrase_end = phrases->select2(basics::get_field(rev_ids,rev_ids_bits,m),&phrase_len);
            phrase_end += phrase_len;
            l = plen;//std::min(plen,phrase_len);
            extracted = _display(phrase_end-l+1,phrase_end);
            std::reverse(extracted,extracted+l);
            int cmp = strncmp((char*)pattern, (char*)extracted, l);
...
void static_selfindex_lz77::_binarySearchSst(unsigned char* pattern, unsigned int plen, unsigned int* left, unsigned int* right){
    int lower = 0;
    int upper = (parsing_len-1)-1;
    unsigned char* extracted;
    unsigned int l;
